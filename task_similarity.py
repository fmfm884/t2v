...
def plot_tsne_embed(embeddings):
    import matplotlib.pyplot as plt
    ...
    task_name = []
    for e in embeddings:
        hessians.append(get_hessian(e))
        task_name.append(e.meta)
        
    data = TSNE().fit_transform(hessians)
    
    for i, name in enumerate(task_name):
        plt.scatter(data[0], data[1], s=20, label=name)
