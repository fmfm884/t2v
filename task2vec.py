...
class Task2Vec:
    ...
    def __init__(self, model: ProbeNetwork, skip_layers=0, max_samples=None, classifier_opts=None,
                 method='montecarlo', method_opts=None, loader_opts=None, bernoulli=False, label=None):
    ...
    def _cache_features(self, dataset: Dataset, indexes=(-1,), max_samples=None, loader_opts: dict = None):
        ...
        for i, (input, target) in tqdm(enumerate(itertools.islice(data_loader, 0, n_batches)), total=n_batches, leave=False, desc="Caching features"):
    ...
    def extract_embedding(self, model: ProbeNetwork):
        ...
        return Embedding(hessian=np.concatenate(hess), scale=np.concatenate(scale), meta=self.label)
